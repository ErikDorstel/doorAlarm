...
void wlanWorker() {
  ...
  if (millis()>=wlanTimer2) { wlanTimer2=millis()+5000;
    if (WiFi.status()!=wifiStatus) { wifiStatus=WiFi.status();
      if (wifiStatus==WL_CONNECTED) { Serial.print("IP: "); Serial.println(WiFi.localIP()); Serial.println("WiFi Connected."); yellowOff(); }
      else { Serial.println("WiFi Disconnected."); yellowOn(); } } } }
